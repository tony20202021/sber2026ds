...
Compares metrics and speed vs LightGBM and LogisticRegression.
"""
import json
import os
import time
...
RESULTS_DIR = "results/solution_2026_04_15_random_forest"
os.makedirs(RESULTS_DIR, exist_ok=True)
OVERALL_PATH = "results/overall.json"
BRANCH = "solution_2026_04_15_random_forest"
...
print("Training RandomForestClassifier (train+val)...")
...
overall.append({
    ...
    "comment": (
        f"RandomForestClassifier (sklearn, n_estimators=300, min_samples_leaf=5). "
        f"Top-{len(TOP_FEATURES)} признаков из LightGBM importance. "
        f"Обучение на train+val, threshold=0.5. "
        f"Цель — сравнение с линейной моделью и LightGBM по метрикам и скорости."
    ),
})
