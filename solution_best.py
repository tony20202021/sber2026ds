"""
solution_best.py — top-100 из solution.py + лучшие новые фичи из solution_2021_old.

Новые фичи (отсутствуют в solution.py, значимы в main.py):
  - per-hour individual shares: hour_share_0..23  (24 фичи)
  - pos_min, pos_max, pos_median                  (3 фичи)
  - neg_min, neg_max, neg_median                  (3 фичи)
Итого: 100 + 30 = 130 фич.
"""
import json
import os
import time
...
RESULTS_DIR = "results/solution_2026_04_15_best_from_all"
os.makedirs(RESULTS_DIR, exist_ok=True)
OVERALL_PATH = "results/overall.json"
BRANCH = "solution_2026_04_15_best_from_all"

# ── Top-100 из feature_importance ────────────────────────────────────────────
with open(OVERALL_PATH) as f:
    _overall = json.load(f)
_fi = next(e for e in _overall if e.get("top_features"))
TOP_100 = _fi["top_features"]
...
def extra_features(df):
    ...
    # per-hour shares через groupby+unstack (memory-efficient)
    hour_shares = (
...
# customer_id order в X совпадает с порядком first-occurrence groupby(sort=False)
# Восстанавливаем порядок клиентов через drop_duplicates на оригинальном df
...
# Reindex extra features в порядке customer_order из X
...
# Целевой набор: top-100 + новые фичи
extra_names = (
...
selected = TOP_100 + extra_names

print(f"Features: top-100={len(TOP_100)}, extra={len(extra_names)}, total={len(selected)}")
...
overall.append({
    ...
    "feature_groups": {
        "top100_solution": len(TOP_100),
        "per_hour_shares": len([f for f in extra_names if "hour_share" in f]),
        "pos_neg_minmax_median": len([f for f in extra_names if "hour_share" not in f]),
    },
    "comment": (
        f"Best-from-all: top-100 (LightGBM gain) + {len(extra_names)} новых фич "
